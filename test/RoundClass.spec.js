...
it('should return the current question', () => {
  expect(round.currentQuestion).toEqual(questions[round.currentIndex]);
});

it('should return a random index that have not be picked yet', () => {
  round.pickedIndexes = [0];
  expect(round.randomIndex).toBe(1);
});
...
it('should set current question to a random one that have not be picked yet', () => {
  ...
  expect(round.currentQuestion).toEqual(questions[1]);
});
...
describe('when submitting an answer', () => {
  ...
  describe('depending on the answer', () => {
    ...
    it('should update the score', () => {
      ...
      round.setQuestion(1);
      round.submitAnswer('up');
      expect(round.score).toEqual([0, 1, 0]);
    });
