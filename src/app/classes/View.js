...
export default class View {
  ...
  bind() {
    ...
    this.$answerLeftButton.addEventListener(
      "click",
      answerButtonListener.bind(this, this.$answerLeftButton),
      false
    );
    this.$answerUpButton.addEventListener(
      "click",
      answerButtonListener.bind(this, this.$answerUpButton),
      false
    );
    this.$answerRightButton.addEventListener(
      "click",
      answerButtonListener.bind(this, this.$answerRightButton),
      false
    );

    this.$replayButton.addEventListener(
      "click",
      replayButtonListener.bind(this),
      false
    );

    const keys = { 37: "left", 38: "up", 39: "right", 32: "spacebar" };
    const buttons = {
      left: this.$answerLeftButton,
      up: this.$answerUpButton,
      right: this.$answerRightButton,
      spacebar: this.$replayButton,
    };
    ...
    this.$normalModeButton.addEventListener(
      "click",
      () => {
    ...
    this.$hardcoreModeButton.addEventListener(
      "click",
      () => {
  ...
  showGameOverScreen(wins, loses, total) {
    ...
    const tweetMessage = encodeURIComponent(
      `Boom! Just made a score of ${wins}/${total}, come and beat me! #logicalornot`
    );
    this.$tweetMyGameButton.setAttribute(
      "href",
      `https://twitter.com/intent/tweet?text=${tweetMessage}&url=${encodeURIComponent(
        "https://gabinaureche.com/logicalornot"
      )}&via=zh0uzi`
    );
